...
import LogoDetails from './components/logo-details'
import VariationsArea from '../components/variations-area/variations-area'
import UndoRedo from './components/undo-redo'
import SaveAndClose from './components/save-and-close'
import DownloadButton from './components/download-button'
import EditProductButton from './components/edit-product-button'
import LogoInspiration from './components/logo-inspiration'
import MyLogosGrid from './components/my-logos-grid'
import ControlsAccordion from './components/controls-accordion'
import IconSettingsPanel from './components/icon-settings-panel'
import ControlsTitle from './components/controls-title'
import SegmentTitle from './components/segment-title'
import InverterButton from './components/inverter-button'
import PlayButton from './components/play-button'
import SegmentsPanel from './components/segments-panel'
import IconSettingsButtons from './components/icon-settings-buttons'
import './lang'
// Helpers
...
import PricingPlanModal from './components/pricing-plan-modal'
...
const LogoMakerEditor = (props) => {
  ...
  // Refs and states
  const [activeSegment, setActiveSegment] = useState(params?.segment || 'name')
  const [unSavedProgress, setUnSavedProgress] = useState(false)
  const [logoSaved, setLogoSaved] = useState(false)
  const [isSavingLogo, setLogoIsSaving] = useState(false)
  ...
  const setSegementActive = useCallback(
    (segment) => {
      setActiveSegment((previousSegment) => {
        ...
        goToWithHistory(navigate, `/editor/${logoId}/${segment}${window.location.search}`, true)
  ...
  useEffect(() => {
    ...
    setActiveSegment(params.segment || 'name')
  }, [props.selectedLogo, params.segment])
  ...
  const goDownloadLogo = React.useCallback(
    (id) => {
      const goDownload = () => goToWithHistory(navigate, `/editor/${id}/download?from_editor=true`)
      if (props.doNotSaveOnDownload) {
        goDownload()
      } else {
        saveLogoToServer(goDownload)
      }
    },
    [props.doNotSaveOnDownload, saveLogoToServer]
  )

  const showPreview = React.useCallback(
    (id) => {
      const goPreview = () => goToWithHistory(navigate, `/editor/${props.selectedLogo.id}/preview`)
      if (props.doNotSaveOnDownload) {
        goPreview()
      } else {
        saveLogoToServer(goPreview)
      }
    },
    [props.doNotSaveOnDownload, saveLogoToServer]
  )
  ...
  const shouldShowChildren = React.useMemo(
    () => !props.LogoInstanceRequired || logoInstance != null,
    [props.LogoInstanceRequired, logoInstance]
  )
...
LogoMakerEditor.propTypes = {
  ...
  doNotSaveOnDownload: PropTypes.bool
}
...
LogoMakerEditor.LogoInspiration = LogoInspiration
...
const LogoMakerEditorExp = LogoMakerEditor

export { EditorContext, LogoMakerEditor, LogoMakerEditorExp as LogoMaker }
